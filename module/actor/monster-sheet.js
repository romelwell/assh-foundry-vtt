 ...
 */
export class AsshActorSheetMonster extends AsshActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["assh", "sheet", "monster", "actor"],
      template: "systems/assh/templates/actors/monster-sheet.html",
      width: 450,
      height: 560,
      resizable: true,
      tabs: [
   ...
   */
  async generateSave() {
    let choices = CONFIG.ASSH.monster_saves;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/assh/templates/actors/dialogs/monster-saves.html",
        templateData
      );
    //Create Dialog window
    new Dialog({
      title: game.i18n.localize("ASSH.dialog.generateSaves"),
      content: dlg,
      buttons: {
        ok: {
  ...
  getData() {
    ...
    // Settings
    data.config.morale = game.settings.get("assh", "morale");
  ...
  async _chooseItemType(choices = ["weapon", "armor", "shield", "gear"]) {
    let templateData = { types: choices },
      dlg = await renderTemplate(
        "systems/assh/templates/items/entity-create.html",
        templateData
      );
    //Create Dialog window
    return new Promise((resolve) => {
      new Dialog({
        title: game.i18n.localize("ASSH.dialog.createItem"),
        content: dlg,
        buttons: {
          ok: {
            label: game.i18n.localize("ASSH.Ok"),
