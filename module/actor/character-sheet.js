import { AsshActor } from "./entity.js";
import { AsshActorSheet } from "./actor-sheet.js";
import { AsshCharacterModifiers } from "../dialog/character-modifiers.js";
import { AsshCharacterCreator } from "../dialog/character-creation.js";

/**
 * Extend the basic ActorSheet with some very simple modifications
 */
export class AsshActorSheetCharacter extends AsshActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["assh", "sheet", "actor", "character"],
      template: "systems/assh/templates/actors/character-sheet.html",
      width: 450,
      height: 530,
      resizable: true,
      tabs: [
  ...
  generateScores() {
    new AsshCharacterCreator(this.actor, {
  ...
  getData() {
    ...
    data.config.initiative = game.settings.get("assh", "initiative") != "group";
    data.config.encumbrance = game.settings.get("assh", "encumbranceOption");
  ...
  async _chooseLang() {
    let choices = CONFIG.ASSH.languages;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/assh/templates/actors/dialogs/lang-create.html",
        templateData
      );
    ...
    return new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          ok: {
            label: game.i18n.localize("ASSH.Ok"),
  ...
  _pushLang(table) {
    ...
    this._chooseLang().then((dialogInput) => {
      const name = CONFIG.ASSH.languages[dialogInput.choice];
      if (update.value) {
  ...
  _onShowModifiers(event) {
    event.preventDefault();
    new AsshCharacterModifiers(this.actor, {
      top: this.position.top + 40,
      left: this.position.left + (this.position.width - 400) / 2,
    }).render(true);
  }
