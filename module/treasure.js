export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("assh", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("assh", "treasure");
    table.object.setFlag("assh", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('ASSH.table.treasure.roll')}</button>`;
...
function drawTreasure(table, data) {
  ...
  data.treasure = {};
  if (table.getFlag('assh', 'treasure')) {
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/assh/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/assh/assets/coins.mp3"
  }
