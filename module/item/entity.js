 ...
 */
export class AsshItem extends Item {
  /* -------------------------------------------- */
  /*    Data Preparation                          */
  /* -------------------------------------------- */
  /**
  ...
  prepareData() {
    ...
    switch (this.data.type) {
      case "spell":
        img = "/systems/assh/assets/default/spell.png";
        break;
      case "ability":
        img = "/systems/assh/assets/default/ability.png";
        break;
      case "armor":
        img = "/systems/assh/assets/default/armor.png";
        break;
      case "weapon":
        img = "/systems/assh/assets/default/weapon.png";
        break;
      case "item":
        img = "/systems/assh/assets/default/item.png";
        break;
    }
  ...
  async rollFormula(options = {}) {
    ...
    return AsshDice.Roll({
      ...
      flavor: game.i18n.format("ASSH.roll.formula", { label: label }),
      title: game.i18n.format("ASSH.roll.formula", { label: label }),
    });
  }
  ...
  getTags() {
    ...
    switch (this.data.type) {
      case "weapon":
        ...
        wTags += formatTag(CONFIG.ASSH.saves_long[data.save], "fa-skull");
        if (data.missile) {
          wTags += formatTag(
        ...
        return wTags;
      case "armor":
        return `${formatTag(CONFIG.ASSH.armor[data.type], "fa-tshirt")}`;
      case "item":
        return "";
      case "spell":
        ...
        if (data.save) {
          sTags += formatTag(CONFIG.ASSH.saves_long[data.save], "fa-skull");
        }
        return sTags;
      case "ability":
        let roll = "";
        roll += data.roll ? data.roll : "";
        roll += data.rollTarget ? CONFIG.ASSH.roll_type[data.rollType] : "";
  ...
  pushTag(values) {
    ...
    if (update) {
      values.forEach((val) => {
        ...
        switch (val) {
          case CONFIG.ASSH.tags.melee:
            newData.melee = true;
            break;
          case CONFIG.ASSH.tags.slow:
            newData.slow = true;
            break;
          case CONFIG.ASSH.tags.missile:
  ...
  async show() {
    ...
    const template = `systems/assh/templates/chat/item-card.html`;
