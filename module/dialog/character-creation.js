...
export class AsshCharacterCreator extends FormApplication {
  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes = ["assh", "dialog", "creator"],
      options.id = 'character-creator';
    options.template =
      'systems/assh/templates/actors/dialogs/character-creation.html';
    options.width = 235;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize('ASSH.dialog.generator')}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label = score != "gold" ? game.i18n.localize(`ASSH.scores.${score}.long`) : "Gold";
    const rollParts = ["3d6"];
    const data = {
      roll: {
        type: "result"
    ...
    return AsshDice.Roll({
      ...
      flavor: game.i18n.format('ASSH.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
      title: game.i18n.format('ASSH.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
    });
  }
  ...
  async close() {
    ...
    const templateData = {
      config: CONFIG.ASSH,
      scores: scores,
      title: game.i18n.localize("ASSH.dialog.generator"),
      stats: this.object.data.stats,
      gold: gold
    }
    const content = await renderTemplate("/systems/assh/templates/chat/roll-creation.html", templateData)
    ChatMessage.create({
      content: content,
      speaker,
    });
  ...
  async _onSubmit(event, { updateData = null, preventClose = false, preventRender = false } = {}) {
    ...
    const itemData = {
      name: "GP",
      type: "item",
      img: "/systems/assh/assets/gold.png",
      data: {
        treasure: true,
        cost: 1,
        weight: 1,
